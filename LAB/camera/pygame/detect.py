...
def get_output_CAM(interpreter, dense_layer_weight, cam_w, cam_h) :
    scaling = False
    CAM = common.output_tensor(interpreter, 0, dense_layer_weight, scaling = scaling)
    if scaling == True :
        pass
    else :
...
def main():
    ...
    custom_model = 'mobilenet_v2_1.0_224_quant_notop_cam.tflite'
    custom_fclayer_w = 'dense_layer_param.pkl'
    with open(os.path.join(custom_model_dir, custom_fclayer_w), 'rb') as f:
        loaded_dense_layer = pickle.load(f)
    ...
    try:
        ...
        while True:
            ...
            interpreter.invoke()
            results = get_output_CAM(interpreter, loaded_dense_layer, cam_w, cam_h)
            ...
            mysurface.blit(cam_surface, (0,0))
